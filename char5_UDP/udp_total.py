...
def Server(ipaddr, port):
    ...
    print('Waitingin {}...'.format(sock.getsockname()))
    while True:
        data, addr = sock.recvfrom(BUFF_SIZE)
        if random.random() < prob:
            print('Message from {} is lost.'.format(addr))
            continue
        # print('{} client message {!r}'. format(addr, data.decode())) # 이걸로 하면 오류남 이미 문자열인데 또 decode를 해서?
        print('{} client message {!r}'. format(addr, data))
    ...
    while True:
        ...
        try:
            ...
            if time > 2.0:
                ...
                # print('Server reply: {!r}'.format(data.decode())) # 입력 개수 만큼 보내자
                print('Server reply: {!r}'.format(data)) # 입력 개수 만큼 보내자
                if index >= sending_counts:
                    break
                index += 1
                time = 0.1
